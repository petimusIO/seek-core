    ...
    def generate_roadmap(self, learner: LearnerProfile) -> List[MicroLesson]:
        ...
        try:
            ...
            if len(roadmap) < min_lessons:
                logger.warning(f"Generated roadmap has fewer than {min_lessons} lessons: {len(roadmap)}")
            elif len(roadmap) > max_lessons:
                logger.warning(f"Generated roadmap has more than {max_lessons} lessons: {len(roadmap)}")
